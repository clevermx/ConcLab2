...
namespace Practik2_1
{
    class Program
    {
        ...
        public static void ImageWork(string filename)
        {
            DateTime dt1, dt2;
            double lAllTime;
            lAllTime = 0;
            ...
            string fname = @"E:/C#/Changed_Images/" + imgName;
            img.Save(fname);
        ...
        public static IEnumerable<string> InitImages(int pFileCount)
        {
            string fname = @"E:/C#/Images/";
            return Directory.EnumerateFiles(fname, "*.jpg").Take(pFileCount) ;
        }
        public static void PerformWithoutPartitioner(int num)
        {
            ...
            try
            {
                ...
                Parallel.ForEach(ImageFiles, Options, ImageWork);
            }
        ...
        public static void PerformWithoutPartitioner(int num, int thrCount)
        {
            cts = new CancellationTokenSource();
            ...
            try
            {
                ...
                {
                    CancellationToken = cts.Token,
                    MaxDegreeOfParallelism = thrCount
                };
                Parallel.ForEach(ImageFiles, Options, ImageWork);
            }
            catch (OperationCanceledException o)
        ...
        public static void PerformBalanced(int num)
        {
            ...
            try
            {
                ...
                var PartedData = Partitioner.Create(ImageFiles.ToArray<string>(), true);
                Parallel.ForEach(PartedData, Options, ImageWork);
            }
            catch (OperationCanceledException o)
            {
                try
            ...
            try
            {
                ...
                Parallel.ForEach(Partitioner.Create(0, num), Options, range => {
                    for (int i = range.Item1; i < range.Item2; i++)
                        ImageWork(ImageFiles.ElementAt(i));
                });
            }
            catch (OperationCanceledException o)
            {
